...
import MaisInfo from "./../modelos/maisInfo";
import { Cpf_Cnpj, Telefone, Cep, PrimeiraMaiusculo } from "./../mascaras/mascaras";
...
export default function Usuarios() {
  ...
  const [orderBy, setOrderBy] = React.useState("apelido");
            ...
            <TableBody>
              ...
              {stableSort(usuarios, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  const nome = PrimeiraMaiusculo(row.nome);
                  const cpf = row.cpf;
                  const tel = row.telefone;
                  const celular = Telefone(tel);
                  const cep = Cep(row.cep);
                  // Primeira em Maiúsculo
                  const apelido = PrimeiraMaiusculo(row.apelido);
